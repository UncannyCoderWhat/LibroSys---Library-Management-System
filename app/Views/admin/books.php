...
<body>
    ...
    <main class="content-workspace">
        ...
        <!-- API Import Modal -->
        <div id="apiImportModal" class="modal" style="display:none"><div class="modal-content" style="max-width:600px"><span class="close" onclick="closeModal('apiImportModal')">&times;</span><h2 class="section-title">API Book Import</h2><p style="font-size:13px;color:#555;margin-bottom:12px;">Enter an ISBN, Google Books ID, or Open Library ID to import a book.</p><form action="index.php?page=admin_books" method="POST" class="api-import-box" style="border-style:solid"><select name="api_source" style="padding:10px;border-radius:6px;border:1px solid #ccc"><option value="isbn">ISBN</option><option value="google_books_id">Google Books ID</option><option value="open_library_id">Open Library ID</option></select><input type="text" name="api_identifier" placeholder="Enter identifier..." required><button type="submit" name="api_import" class="submit-btn"><i class="fa-solid fa-download"></i> Import</button></form></div></div>

        <!-- Search and Filter -->
        <section class="activity-section" style="padding:12px 0; margin-top: 0;"><div class="search-filter-container"><div class="search-box"><i class="fa-solid fa-magnifying-glass"></i><input type="text" id="bookSearch" placeholder="Search by title, author, ISBN..."></div><div class="filter-box"><select id="statusFilter"><option value="all">All Status</option><option value="available">Available</option><option value="unavailable">Unavailable</option><option value="archived">Archived</option></select><select id="categoryFilter"><option value="all">All Categories</option><?php foreach($all_categories??[] as $cat):?><option value="<?php echo htmlspecialchars($cat['name']);?>"><?php echo htmlspecialchars($cat['name']);?></option><?php endforeach;?></select><select id="bookTypeFilter"><option value="all">All Types</option><?php foreach($all_book_types??[] as $bt):?><option value="<?php echo htmlspecialchars($bt);?>"><?php echo htmlspecialchars($bt);?></option><?php endforeach;?></select><select id="genreFilter"><option value="all">All Genres</option><option>Fiction</option><option>Non-Fiction</option><option>Mystery</option><option>Sci-Fi</option><option>Fantasy</option><option>Romance</option><option>Horror</option><option>History</option><option>Biography</option><option>Thriller</option><option>Adventure</option><option>Drama</option><option>Poetry</option><option>Comic</option><option>Graphic Novel</option><option>Action</option><option>Comedy</option><option>Crime</option><option>Dystopian</option><option>Fairy Tale</option><option>Gothic</option><option>Memoir</option><option>Philosophy</option><option>Psychology</option><option>Science</option><option>Self-Help</option><option>Short Story</option><option>Suspense</option><option>Teen Fiction</option><option>True Crime</option><option>Western</option><option>Young Adult</option></select></div></div></section>

        <!-- Books Table -->
        <section class="activity-section" style="margin-top: 0;"><div class="section-header"><h2 class="section-title" style="margin:0"><i class="fa-solid fa-book"></i> Library Circulation</h2><span style="font-size:13px;color:#888" id="resultCount"><?php echo count($all_books??[]);?> book(s)</span></div>
        <div class="books-table-container"><table class="books-table" id="booksTable"><thead><tr><th>Cover</th><th>Title</th><th>Author</th><th>ISBN</th><th>Type</th><th>Genre</th><th>Publisher</th><th>Year</th><th>Language</th><th>Copies</th><th>Status</th><th>Actions</th></tr></thead><tbody id="booksTableBody">
        <?php if(empty($all_books)):?><tr><td colspan="13" style="text-align:center;padding:30px;color:#999">No books found. Add a new book to get started.</td></tr>
        <?php else: foreach($all_books as $book):?><tr class="book-row" data-title="<?php echo strtolower(htmlspecialchars($book['title']));?>" data-author="<?php echo strtolower(htmlspecialchars($book['author']));?>" data-isbn="<?php echo strtolower(htmlspecialchars($book['isbn']));?>" data-genre="<?php echo strtolower(htmlspecialchars($book['genre']));?>" data-book_type="<?php echo strtolower(htmlspecialchars($book['book_type']??''));?>" data-category="<?php echo strtolower(htmlspecialchars($book['category_name']??''));?>" data-status="<?php echo htmlspecialchars($book['status']??'available');?>">
        ...
        <td><?php echo htmlspecialchars($book['author']);?></td><td><?php echo htmlspecialchars($book['isbn']);?></td><td><?php echo htmlspecialchars($book['book_type']??'');?></td><td><?php $genres = array_map('trim', explode(',', $book['genre']??'')); foreach($genres as $g): if(!empty($g)): ?><span class="genre-chip-static"><?php echo htmlspecialchars($g);?></span><?php endif; endforeach;?></td><td><?php echo htmlspecialchars($book['publisher']??'');?></td><td><?php echo htmlspecialchars($book['publication_year']??'');?></td><td><?php echo htmlspecialchars($book['language']??'English');?></td><td><?php echo (int)($book['copies']??1);?></td>
        ...
        <td><div class="dropdown-actions">
            ...
            <div class="dropdown-menu">
                ...
                <button onclick='openMangaChaptersModal(<?php echo (int)$book['id'];?>,"<?php echo htmlspecialchars(addslashes($book['title']));?>")'><i class="fa-solid fa-images"></i> Chapters</button>
    ...
    <div class="genre-selector-wrapper"><label>Genre</label><div class="genre-chip-container"><input type="hidden" name="genre" id="add_genre_input" value=""><div class="genre-chips"><button type="button" class="genre-chip" data-genre="Fiction">Fiction</button><button type="button" class="genre-chip" data-genre="Non-Fiction">Non-Fiction</button><button type="button" class="genre-chip" data-genre="Mystery">Mystery</button><button type="button" class="genre-chip" data-genre="Sci-Fi">Sci-Fi</button><button type="button" class="genre-chip" data-genre="Fantasy">Fantasy</button><button type="button" class="genre-chip" data-genre="Romance">Romance</button><button type="button" class="genre-chip" data-genre="Horror">Horror</button><button type="button" class="genre-chip" data-genre="History">History</button><button type="button" class="genre-chip" data-genre="Biography">Biography</button><button type="button" class="genre-chip" data-genre="Thriller">Thriller</button><button type="button" class="genre-chip" data-genre="Adventure">Adventure</button><button type="button" class="genre-chip" data-genre="Drama">Drama</button><button type="button" class="genre-chip" data-genre="Poetry">Poetry</button><button type="button" class="genre-chip" data-genre="Comic">Comic</button><button type="button" class="genre-chip" data-genre="Graphic Novel">Graphic Novel</button><button type="button" class="genre-chip" data-genre="Action">Action</button><button type="button" class="genre-chip" data-genre="Comedy">Comedy</button><button type="button" class="genre-chip" data-genre="Crime">Crime</button><button type="button" class="genre-chip" data-genre="Dystopian">Dystopian</button><button type="button" class="genre-chip" data-genre="Fairy Tale">Fairy Tale</button><button type="button" class="genre-chip" data-genre="Gothic">Gothic</button><button type="button" class="genre-chip" data-genre="Memoir">Memoir</button><button type="button" class="genre-chip" data-genre="Philosophy">Philosophy</button><button type="button" class="genre-chip" data-genre="Psychology">Psychology</button><button type="button" class="genre-chip" data-genre="Science">Science</button><button type="button" class="genre-chip" data-genre="Self-Help">Self-Help</button><button type="button" class="genre-chip" data-genre="Short Story">Short Story</button><button type="button" class="genre-chip" data-genre="Suspense">Suspense</button><button type="button" class="genre-chip" data-genre="Teen Fiction">Teen Fiction</button><button type="button" class="genre-chip" data-genre="True Crime">True Crime</button><button type="button" class="genre-chip" data-genre="Western">Western</button><button type="button" class="genre-chip" data-genre="Young Adult">Young Adult</button></div></div></div>
    <div><label>Book Type</label><select name="book_type" id="add_book_type"><option value="">-- Select Type --</option><?php foreach($all_book_types??[] as $bt):?><option value="<?php echo htmlspecialchars($bt);?>"><?php echo htmlspecialchars($bt);?></option><?php endforeach;?></select></div>
    <div id="add_manga_hint" style="display:none;font-size:12px;color:#e94560;margin-top:4px;"><i class="fa-solid fa-circle-info"></i> Manga books require chapter uploads. Use the Chapters action after saving.</div>
    ...
    <div class="genre-selector-wrapper"><label>Genre</label><div class="genre-chip-container"><input type="hidden" name="genre" id="edit_genre_input" value=""><div class="genre-chips" id="edit_genre_chips"><button type="button" class="genre-chip" data-genre="Fiction">Fiction</button><button type="button" class="genre-chip" data-genre="Non-Fiction">Non-Fiction</button><button type="button" class="genre-chip" data-genre="Mystery">Mystery</button><button type="button" class="genre-chip" data-genre="Sci-Fi">Sci-Fi</button><button type="button" class="genre-chip" data-genre="Fantasy">Fantasy</button><button type="button" class="genre-chip" data-genre="Romance">Romance</button><button type="button" class="genre-chip" data-genre="Horror">Horror</button><button type="button" class="genre-chip" data-genre="History">History</button><button type="button" class="genre-chip" data-genre="Biography">Biography</button><button type="button" class="genre-chip" data-genre="Thriller">Thriller</button><button type="button" class="genre-chip" data-genre="Adventure">Adventure</button><button type="button" class="genre-chip" data-genre="Drama">Drama</button><button type="button" class="genre-chip" data-genre="Poetry">Poetry</button><button type="button" class="genre-chip" data-genre="Comic">Comic</button><button type="button" class="genre-chip" data-genre="Graphic Novel">Graphic Novel</button><button type="button" class="genre-chip" data-genre="Action">Action</button><button type="button" class="genre-chip" data-genre="Comedy">Comedy</button><button type="button" class="genre-chip" data-genre="Crime">Crime</button><button type="button" class="genre-chip" data-genre="Dystopian">Dystopian</button><button type="button" class="genre-chip" data-genre="Fairy Tale">Fairy Tale</button><button type="button" class="genre-chip" data-genre="Gothic">Gothic</button><button type="button" class="genre-chip" data-genre="Memoir">Memoir</button><button type="button" class="genre-chip" data-genre="Philosophy">Philosophy</button><button type="button" class="genre-chip" data-genre="Psychology">Psychology</button><button type="button" class="genre-chip" data-genre="Science">Science</button><button type="button" class="genre-chip" data-genre="Self-Help">Self-Help</button><button type="button" class="genre-chip" data-genre="Short Story">Short Story</button><button type="button" class="genre-chip" data-genre="Suspense">Suspense</button><button type="button" class="genre-chip" data-genre="Teen Fiction">Teen Fiction</button><button type="button" class="genre-chip" data-genre="True Crime">True Crime</button><button type="button" class="genre-chip" data-genre="Western">Western</button><button type="button" class="genre-chip" data-genre="Young Adult">Young Adult</button></div></div></div>
    <div><label>Book Type</label><select name="book_type" id="edit_book_type"><option value="">-- Select Type --</option><?php foreach($all_book_types??[] as $bt):?><option value="<?php echo htmlspecialchars($bt);?>"><?php echo htmlspecialchars($bt);?></option><?php endforeach;?></select></div>
    <div id="edit_manga_hint" style="display:none;font-size:12px;color:#e94560;margin-top:4px;"><i class="fa-solid fa-circle-info"></i> Manga books require chapter uploads. Use the Chapters action after saving.</div>
    ...
    <!-- AUTHOR BOOKS MODAL (AJAX loaded) -->
    <div id="authorBooksModal" class="modal modal-large" style="display:none"><div class="modal-content"><span class="close" onclick="closeModal('authorBooksModal')">&times;</span><div id="authorBooksModalContent"><p style="text-align:center;padding:20px;">Loading...</p></div></div></div>

    <!-- PUBLISHER BOOKS MODAL (AJAX loaded) -->
    <div id="publisherBooksModal" class="modal modal-large" style="display:none"><div class="modal-content"><span class="close" onclick="closeModal('publisherBooksModal')">&times;</span><div id="publisherBooksModalContent"><p style="text-align:center;padding:20px;">Loading...</p></div></div></div>
    ...
    <!-- MANGA CHAPTERS MODAL (AJAX loaded) -->
    <div id="mangaChaptersModal" class="modal modal-large" style="display:none"><div class="modal-content"><span class="close" onclick="closeModal('mangaChaptersModal')">&times;</span><div id="mangaChaptersModalContent"><p style="text-align:center;padding:20px;">Loading...</p></div></div></div>
